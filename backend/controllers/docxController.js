...
module.exports.getAnket = async function(req, res) {
  try {
    ...
    const faculty = await db.getFaculty(user.Faculty);
    const criterias = JSON.parse((await db.getCriterias(user.Faculty)).Crits);

    const zip = new Zip();

    /* читаем файлы архива в память */
    const f01 = fs.readFileSync(anketPath + '/docs/Anketa/_rels/.rels');
    const f02 = fs.readFileSync(anketPath + '/docs/Anketa/docProps/app.xml');
    const f03 = fs.readFileSync(anketPath + '/docs/Anketa/docProps/core.xml');
    const f04 = fs.readFileSync(anketPath + '/docs/Anketa/word/_rels/document.xml.rels');
    const f05 = fs.readFileSync(anketPath + '/docs/Anketa/word/fontTable.xml');
    let f06 = fs.readFileSync(anketPath + '/docs/Anketa/word/document.xml');
    const f07 = fs.readFileSync(anketPath + '/docs/Anketa/word/header1.xml');
    const f08 = fs.readFileSync(anketPath + '/docs/Anketa/word/header2.xml');
    const f09 = fs.readFileSync(anketPath + '/docs/Anketa/word/numbering.xml');
    const f10 = fs.readFileSync(anketPath + '/docs/Anketa/word/settings.xml');
    const f11 = fs.readFileSync(anketPath + '/docs/Anketa/word/styles.xml');
    const f12 = fs.readFileSync(anketPath + '/docs/Anketa/[Content_Types].xml');
    const f13 = fs.readFileSync(anketPath + '/docs/Anketa/word/theme/theme1.xml');
    const f14 = fs.readFileSync(anketPath + '/docs/Anketa/word/webSettings.xml');
